...
uint16_t long nonVolatileCount,MaxCount,error,requiredValue = 0;

//updates in the ISR:
volatile long long count;

double Kd = 0, Kp = 5, Ki = 0.0001, errorIN = 0, previousError, derivative;
int output2;
int output1;
double PID;
...
void loop() {
  ...
  error = abs(requiredValue-MaxCount);
  dt = millis()-lastStep;
  if(dt>=5){
    lastStep = millis();
  ...
  if (requiredValue > MaxCount)
  {
    output1 = constrain(PID, 0, 50);
    output2 = 0;
  }
  else if (requiredValue < MaxCount)
  {
